<template>
  <ul class="segment-ip-input" ref="ipAddr">
 ...
 * ip地址输入组件（分段式）
 * 调用示例：
 * <ip-input v-model="colonyIp"></ip-input>
...
export default {
  ...
  methods: {
    ...
    compositionstart(e, index) {
      this.beforePosition = e.target.selectionStart
    ...
    compositionend(e, index) {
      ...
      if (!this.shouldRemoveText) {
        e.target.selectionStart = this.beforePosition
        e.target.selectionEnd = this.beforePosition
      }
    },
    keyup(e, index, item) {
      ...
      switch (e.key) {
        ...
        case 'ArrowLeft':
          ...
          break
        case 'Tab':
          // 聚焦到下一个segment-ip-input
          const fanIpAddrs = document.getElementsByClassName('segment-ip-input')
